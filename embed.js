...
function embed() {
	...
	const trickyDarkStyle = [
		...
		"xt256",
		"zenburn",
	]; // dark styles without 'dark', 'black' or 'night' in its name
	const isDarkStyle =
		style.includes("dark") ||
		style.includes("black") ||
		style.includes("night") ||
		trickyDarkStyle.includes(style);
...
`);
	...
	const loadHLJS =
		typeof hljs != "undefined" && typeof hljs.highlightBlock != "undefined"
			? Promise.resolve()
			: loadScript(HLJSURL);
	let loadHLJSNum;
	if (showLineNumbers) {
	...
	Promise.all(
		showLineNumbers
			? [fetchFile, loadHLJS, loadHLJSNum]
			: [fetchFile, loadHLJS],
	)
		.then((result) => {
...
delivered <span class="hide-in-phone">with ❤ </span>by <a target="_blank" href="${serviceProvider}">Hsieh-Ting Lin M.D.</a>`;
