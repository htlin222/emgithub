  ...
  <div class="emgithub-file emgithub-file-${isDarkStyle ? "dark" : "light"}"
    ...
    <div class="file-data ${styleClassName}">
      ${
        ...
        ${
					showCopy
						? `<a class="copy-btn copy-btn-${isDarkStyle ? "dark" : "light"}" href="javascript:void(0)">Copy</a>`
						: ""
				}
    ...
    ${
			showFileMeta
				? `<div class="file-meta file-meta-${isDarkStyle ? "dark" : "light"}"
      ...
      delivered <span class="hide-in-phone">with ❤ </span>by <a target="_blank" href="${serviceProvider}">Hsieh-Ting Lin</a>
    </div>`
				: ""
		}
...
`,
	...
	const loadHLJS =
		typeof hljs != "undefined" && typeof hljs.highlightElement != "undefined"
			? Promise.resolve()
			: loadScript(HLJSURL);
	...
	const loadHLJSNum = loadHLJS.then(() =>
		typeof hljs.lineNumbersBlock != "undefined"
			? Promise.resolve()
			: loadScript(HLJSNumURL),
	);
	const loadHLJSStyle = fetch(
	...
	if (type === "markdown" || type === "ipynb") {
		const loadMarked =
			typeof marked != "undefined"
				? Promise.resolve()
				: loadScript(
						"https://cdn.jsdelivr.net/npm/marked@4.0.18/marked.min.js",
					);
		const loadMarkdownStyle = fetch(
		...
		const loadKatexStyle = fetch(
			...
			.then((text) => {
				insertStyle(
					text.replaceAll(
						"url(fonts/",
						"url(https://cdn.jsdelivr.net/npm/katex@0.16.0/dist/fonts/",
					),
				);
			});
		...
		if (type === "ipynb") {
			const loadAnsiUp =
				typeof AnsiUp != "undefined"
					? Promise.resolve()
					: loadScript(
							"https://cdn.jsdelivr.net/gh/drudru/ansi_up@4.0.4/ansi_up.min.js",
						);
			const loadDOMPurify =
			...
			const loadNotebookjs = Promise.all([
				loadMarked,
				loadAnsiUp,
				loadDOMPurify,
			])
				.then(() =>
					typeof nb != "undefined"
						? Promise.resolve()
						: loadScript(
								"https://cdn.jsdelivr.net/gh/jsvine/notebookjs@0.6.7/notebook.min.js",
							),
				)
				.then(() => {
					nb.markdown = (text) =>
						marked.parse(text, { baseUrl: rawDirectoryURL });
	...
	Promise.allSettled(promises).then((result) => {
		...
		if (type === "code") {
			...
			hljs.lineNumbersBlock(codeTag, {
				singleLine: true,
				startFrom:
					startLine > 0 && fetchSuccess ? Number.parseInt(startLine) : 1,
			});
		} else if (type === "markdown") {
			targetDiv.querySelector(".html-area").innerHTML = fetchSuccess
				? marked.parse(result[0].value, { baseUrl: rawDirectoryURL })
				: result[0].reason;
		} else if (type === "ipynb") {
			try {
				if (fetchSuccess) {
		...
		if (type === "markdown" || type === "ipynb") {
			...
			const loadKatexAutoRender = loadKatex.then(() =>
...
function scopeCss(styleText, scopeSelector) {
	...
	for (const rule of styleElement.sheet.cssRules) {
		if (rule.constructor.name === "CSSStyleRule") {
			...
			const cssSelectorPrepended = cssSelector
				.split(",")
